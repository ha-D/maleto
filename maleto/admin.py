...
def lang_start(update, context):
    ...
    update.message.reply_text('Select your language', parse_mode=ParseMode.MARKDOWN, reply_markup=btns)
    return LANG_SEL
...
def lang_select(update, context):
    ...
    if lang not in langs.keys():
        update.message.reply_text('Language not recognized', parse_mode=ParseMode.MARKDOWN, reply_markup=ReplyKeyboardRemove())

    else:
        with User.find_by_id(update.effective_user.id) as user:
            user.lang = langs[lang]
    update.message.reply_text('Language changed', parse_mode=ParseMode.MARKDOWN, reply_markup=ReplyKeyboardRemove())
