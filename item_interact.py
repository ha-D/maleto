...
def imes_owner_default(item, user_id):
    n = InlineKeyboardMarkup([
        [
            InlineKeyboardButton("Publish", callback_data=PublishCallback.data(item.id))
        ],
        [
            InlineKeyboardButton("🗑  Delete", callback_data=DeleteCallback.data(item.id)),
            InlineKeyboardButton("✏️  Edit", callback_data=EditCallback.data(item.id)),
        ]
    ])
...
def imes_owner_publishing(item, user_id):
    ...
    buttons = [InlineKeyboardButton("◀️ Back", callback_data=PublishCallback.data(item.id, 'cancel'))]
    for chat in user.chats: 
        ...
        buttons.append(InlineKeyboardButton(btn_msg, callback_data=PublishCallback.data(item.id, action, chat['chat_id'])))
    return item.generate_owner_message(), InlineKeyboardMarkup([[b] for b in buttons])


def imes_owner_deleting(item, user_id):
    msg = '\n'.join([
        item.get_owner_message(),
        'Are you sure you want to delete this item? 🙀'
    ])
    btns = InlineKeyboardMarkup([[
        InlineKeyboardButton("Yes", callback_data=DeleteCallback.data(item.id, 'yes')),
        InlineKeyboardButton("No", callback_data=DeleteCallback.data(item.id, 'no')),
    ]])
    return msg, btns
...
def imes_buyer(imes, item, user_id):
    ...
    btns = InlineKeyboardMarkup([[InlineKeyboardButton("I don't want it anymore", callback_data=RevokeBidCallback.data(item.id))]])
    return msg, btns
...
def imes_in_waiting_list(imes, item, user_id):
    ...
    btns = InlineKeyboardMarkup([
        [   
            InlineKeyboardButton("Make Higher Offer", callback_data=BidCallback.data(item.id)),
            InlineKeyboardButton("I don't want it anymore", callback_data=RevokeBidCallback.data(item.id)),
        ],
    ])
    return msg, btns
...
def imes_no_bidders(imes, item, user_id):
    ...
    btns = InlineKeyboardMarkup([
        [   
            InlineKeyboardButton("Buy with this price", callback_data=WaitListCallback.data(item.id, price)),
            InlineKeyboardButton("Make Higher Offer", callback_data=BidCallback.data(item.id)),
        ],
    ])
    return msg, btns
...
def imes_not_bidding(imes, item, user_id):
    ...
    btns = InlineKeyboardMarkup([
        [   
            InlineKeyboardButton("Go in queue with this price", callback_data=WaitListCallback.data(item.id, price)),
            InlineKeyboardButton("Make Higher Offer", callback_data=BidCallback.data(item.id)),
        ],
    ])
    return msg, btns
